(() => {
  ...
  var main_page = class {
    ...
    async init(params) {
      const tableBody = jq(".pdt-main #presenter-table tbody");
  ...
  var link_modal = class {
    ...
    async init(table, db, host) {
      ...
      table.off("click.pdtLinkModal", ".link-button").on("click.pdtLinkModal", ".link-button", async (event) => {
        ...
        const presenterId = Number(jq(button).data("id"));
        ...
        jq(".pdt-link-armember #id-stat").text("This presenter is not linked to any ARMember account.");
      ...
      jq(".pdt-link-armember #link-cancel").off("click.pdtLinkModal").on("click.pdtLinkModal", () => {
        linkModalShadow.prop("hidden", true);
        this.resetLinkSelection(linkMemberBtn, defaultPlaceholder);
      });
      if (!this.presenterSelect) {
      ...
      linkMemberBtn.off("click.pdtLinkModal").on("click.pdtLinkModal", () => {
        ...
        jq(this.buttonObject).val(this.selectedID);
        jq(".pdt-link-armember #id-stat").text(`ARMember[ ID: ${this.selectedID} ]`);
        linkModalShadow.prop("hidden", true);
        this.resetLinkSelection(linkMemberBtn, defaultPlaceholder);
      });
    }
  ...
  var new_member = class {
    ...
    async init(db, refreshTable) {
      ...
      jq(".pdt-main #add-member").off("click.pdtNewMember").on("click.pdtNewMember", () => {
        nMemberShadow.prop("hidden", false);
        nMemberModal.prop("hidden", false);
      });
      jq(".pdt-add-new-member #n-member-cancel").off("click.pdtNewMember").on("click.pdtNewMember", () => {
        closeModal();
      });
      ...
      jq(".pdt-add-new-member #n-member-submit").off("click.pdtNewMember").on("click.pdtNewMember", async () => {
        var _a, _b, _c, _d, _e, _f, _g, _h;
        const firstName = (_b = (_a = jq(".pdt-add-new-member #f_name").val()) == null ? void 0 : _a.toString().trim()) != null ? _b : "";
        const lastNameInput = jq(".pdt-add-new-member #l_name");
        const emailInput = jq(".pdt-add-new-member #email");
        const phone = (_d = (_c = jq(".pdt-add-new-member #p_number").val()) == null ? void 0 : _c.toString().trim()) != null ? _d : "";
  ...
  var presenter_check = class {
    ...
    async init(db, host, refreshTable) {
      ...
      this.host = host;
      this.refreshTable = refreshTable;
      const regPresShadow = jq("#pdt-shadow-registered-attendee");
      const regPresModal = jq("#pdt-attendee-check");
      const regPresenterSubmitBtn = jq("#pdt-attendee-check #reg-attendee-submit");
      ...
      jq("#pdt-n-member-modal #r_presenter").off("click.pdtPresenterCheck").on("click.pdtPresenterCheck", () => {
        this.resetPresenterSelection(regPresenterSubmitBtn, presenterDefaultPlaceholder);
        regPresShadow.prop("hidden", false);
        regPresModal.prop("hidden", false);
      });
      jq("#pdt-attendee-check #reg-attendee-cancel").off("click.pdtPresenterCheck").on("click.pdtPresenterCheck", () => {
        hidePresenterCheck();
      });
      ...
      regPresenterSubmitBtn.off("click.pdtPresenterCheck").on("click.pdtPresenterCheck", async () => {
        ...
        hidePresenterCheck();
        jq("#pdt-shadow-new-member").prop("hidden", true);
        jq("#pdt-n-member-modal").prop("hidden", true);
      });
    }
  };
  ...
  (function() {
    const jq2 = window.jQuery || window.jquery || window.$;
