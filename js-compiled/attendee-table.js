  ...
  function event_hookups() {
    {
      ...
      document.querySelectorAll(".link_account_btn").forEach((button) => {
        button.addEventListener("click", () => link_account_btn(jq(button)));
      });
    }
    const new_member_modal = jq(".pdt-add-new-member");
    const new_member_modal_wrapper = jq("#pdt-modal-wrapper-for-add-new-member");
    const registered_modal = jq(".pdt-presenter-check");
    const registered_modal_wrapper = jq("#pdt-modal-wrapper-for-registered-presenter");
    {
      jq("#pdt-add-member").on("click", function() {
      ...
      jq("#pdt-is-presenter-btn").on("click", function() {
        registered_modal.prop("hidden", false);
        registered_modal_wrapper.prop("hidden", false);
        new_member_modal.prop("hidden", true);
        new_member_modal_wrapper.prop("hidden", true);
      });
      jq("#pdt-new-member-cancel").click(function(e) {
        e.preventDefault();
        new_member_modal.prop("hidden", true);
        new_member_modal_wrapper.prop("hidden", true);
      });
      jq("#pdt-new-member-submit").click(function() {
        var _a, _b, _c, _d;
        let f_name = String((_a = jq("#pdt-f_name").val()) != null ? _a : "").trim();
        let l_name = String((_b = jq("#pdt-l_name").val()) != null ? _b : "").trim();
        let email = String((_c = jq("#pdt-email").val()) != null ? _c : "").trim();
        let ph_number = String((_d = jq("#pdt-ph_number").val()) != null ? _d : "").trim();
        const errors = [];
...
- ${errors.join("\n- ")}`);
        ...
        jq("#pdt-l_name").val("");
        jq("#pdt-email").val("");
        jq("#pdt-ph_number").val("");
        new_member_modal.prop("hidden", true);
        new_member_modal_wrapper.prop("hidden", true);
      });
    }
    {
      jq("#pdt-registered-cancel").click(function() {
        new_member_modal.prop("hidden", false);
        new_member_modal_wrapper.prop("hidden", false);
      ...
      jq("#pdt-p_name").keyup(function(e) {
        let value = jq(this).val();
        let dropdown = jq("#pdt-presenter-options");
      ...
      jq("#pdt-presenter-options").on("click", ".pdt-presenter-option", function() {
        var _a, _b;
        selectedPresenterId = Number((_a = jq(this).attr("data-id")) != null ? _a : -1);
        if (selectedPresenterId === -1) {
          return;
        }
        const selectedName = String((_b = jq(this).attr("data-name")) != null ? _b : "");
        jq("#pdt-p_name").val(selectedName);
        jq("#pdt-presenter-options").empty().prop("hidden", true);
      });
    }
    const linkARMemerModal = jq("#pdt-link-armember");
    {
      jq("#pdt-link-armember-btn").click(function() {
        linkARMemerModal.prop("hidden", true);
      });
    }
  }
  function load_Attendees_Table(datas) {
    const table_body = jq("#pdt-attendee-table tbody");
    table_body.empty();
  ...
  function link_account_btn(button) {
    ...
    console.log(person);
    jq("#pdt-link-armember").prop("hidden", false);
  }
